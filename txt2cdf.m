...
%
% NOTE: This requires table functionality inorder to read the variable
% names and rename them dynamical. This means that this will only work on
% matlab 2014a.
% 
% Current inputs:
...
addParamValue(p,'outputPath','default'); %#ok<*NVREPL>
addParamValue(p,'timeOffset','default');
addParamValue(p,'delimiter','\t');
addParamValue(p,'headerLines',0);
...
% then Lux will not be able to be calculated if missing. CLA requires
% either RGB or CS to be present, while CS can only be calculated from CLA.
% To this end if CS is unable to be calculated then this program will error
% out.
...
%% calculate CLA and put in to output
if has.CLA == false
	if has.CS == true
...
%% calculate CS and put in to output
if has.CS == false
	if has.CLA == true
...
% using the file name this will find the device number, if the file name
...
% it will then ask the user to input the device number. This is a required
